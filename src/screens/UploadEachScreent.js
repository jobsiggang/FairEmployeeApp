...
  saving, uploading, imagesLength, selectedImage, // 👈 uploading 상태를 받습니다.
}) => (
  <View>
...
        disabled={uploading} // 👈 uploading이 true일 때 비활성화
      >
...
      </TouchableOpacity>
      <TouchableOpacity 
        style={[styles.compactButton, styles.kakaoBtn, !selectedImage && styles.buttonDisabled]} // 🚨 비활성화 스타일 적용
        onPress={onShare}
        isabled={!selectedImage}// 🚨 selectedImage가 null/false일 때 비활성화
      >
...
const UploadEachScreen = ({ navigation, route }) => {
  ...
  const [rotationPending, setRotationPending] = useState(false); // 회전 적용 대기 상태
  ...
  // const { CANVAS_WIDTH: C_W, CANVAS_HEIGHT: C_H, entries, tableConfig } = sharedLogic;
// ✅ 수정 (안전하게 기본값 []와 {}를 할당):
const { user, forms, selectedForm, formData, validateForm, updateField, onDateChange, setDatePickerField,validationErrors } = sharedLogic;
      ...
      if (!data?.success) {
        ...
        if (dbData?.success) {
          setUploadedThumbnails(prev => {
                const newThumbnails = [thumbnailData, ...prev];
                // 배열 길이가 20을 초과하면 가장 오래된 항목(배열의 끝)을 잘라냅니다.
...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 0, marginBottom: 16 }}>
                {uploadedThumbnails.map((thumb, idx) => (
                  <Image
                    key={idx}
                    source={{ uri: thumb }}
                    style={{ width: 120, height: 90, borderRadius: 8, marginRight: 12, marginBottom: 12, borderWidth: 2, borderColor: '#2563eb' }}
                  />
                ))}
              </View>
            )}
